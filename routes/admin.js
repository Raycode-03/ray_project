  ...
  data.imageUrl = '';
        ...
        try {
            ...
            const cloudinaryResult = await cloudinary.uploader.upload(image_path, {
                ...
                width: 800, // Example: set a maximum width
                crop: "limit" //
...
router.post('/products/update/:id', imageupload, async (req, res, next) => {
  ...
  try {
    ...
    let data = { title, summary, price: +price, description };

    let oldPublicId = existingProduct.publicId || null;

    if (!req.file) {
      // No new image uploaded, keep old image data
      data.imageUrl = existingProduct.imageUrl;
      data.publicId = existingProduct.publicId;
    } else {
      try {
        // Upload new image to Cloudinary
        const cloudinaryResult = await cloudinary.uploader.upload(image_path, {
          folder: "ray_node_products",
          quality: "auto", // q_auto
          fetch_format: "auto", // f_auto
          width: 800, // Example: set a maximum width
          crop: "limit" //
        ...
        console.error("Cloudinary Upload Error:", cloudinaryErr);
        try {
          if (image_path) await fs.unlink(image_path);
        } catch (e) {
    ...
    await db.get_gb().collection('products').updateOne({ _id: id }, { $set: data });
